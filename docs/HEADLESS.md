...
| Live mic (`cpal`) | Workspace: **not compiled** (`mic` off). Desktop-edge: **on by default**; `mic_smoke` after Voice clearance |
...
Then **Mic smoke (live)** in the UI. (`scripts\tauri-dev-mic.cmd` also works;
mic is already the desktop-edge default.)
